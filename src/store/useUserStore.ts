...
export const useUsersStore = defineStore("user", {
  ...
  actions: {
    async signIn(user: any) {
      try {
        await axios.post("/api/wp/v2/register/user", user, {
          headers: {
    ...
    async login(user: any) {
      ...
      try {
        const response = await axios.post("/api/jwt-auth/v1/token", user, {
    ...
    async fetchUserInfo() {
      ...
      try {
        const response = await axios.get("/api/wp/v2/users/me", {
          headers: {
            Authorization: `Bearer ${this.users.token}`,
          },
        });
